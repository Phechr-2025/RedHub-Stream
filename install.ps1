...
$envFileName = Get-ConfigValue $cfg "env_file" $DefaultEnvFile
...
"@ | Set-Content -Encoding UTF8 $psShim
  ...
  Write-Host "โปรเจกต์: $projectName"
  Write-Host "เวอร์ชัน (tag ล่าสุด): $version"
  Write-Host "ช่องทาง release: $releaseChannel"
  Write-Host "ติดตั้งพร้อมใช้งาน: $installPath"
  Write-Host "คำสั่งหลัก: $installPath\menuwed.py"
  Write-Host "เรียกใช้งานได้ทันที: menuwed"
