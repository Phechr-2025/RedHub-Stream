...
def send_brevo_otp(email: str, code: str, purpose: str) -> bool:
    ...
    # แสดงรหัส OTP เป็นกล่องแยกทีละหลักในตารางเดียวแถวเดียว (ไม่ใช้ช่องว่างจริงคั่นตัวเลข
    # เพราะ Gmail/แอปอีเมลบางตัวจะตัดบรรทัดตรงช่องว่างเมื่อพื้นที่แคบ ทำให้ตัวเลขล้นไปอีกบรรทัด)
    digit_cells = "".join(
        f'<td style="padding:0 4px;">'
        f'<div style="width:38px;height:48px;background-color:#0f1115;border:1px solid #6c5ce7;'
        f'border-radius:8px;color:#ffffff;font-size:26px;font-weight:700;'
        f'font-family:\'Courier New\',monospace;line-height:48px;text-align:center;">{digit}</div>'
        f"</td>"
        for digit in code
    )
    html = f"""
