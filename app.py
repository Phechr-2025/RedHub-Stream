...
REGISTER_FLOW_ENDPOINTS = {
    "user_register", "user_register_verify", "user_register_resend_otp", "user_register_cancel",
}
RECOVER_FLOW_ENDPOINTS = {
    "recover_account", "recover_otp", "recover_resend_otp", "recover_cancel", "recover_reset",
}
EMAIL_FLOW_ENDPOINTS = {
    "user_email", "user_email_verify", "user_email_resend_otp", "user_email_cancel",
}
...
def user_register():
    ...
    # ขั้นยืนยัน OTP หลังส่งฟอร์มสมัคร
    if session.get("register_otp_challenge"):
        conn = get_db_connection()
        challenge = get_session_otp(conn, "register_otp_challenge")
        conn.close()
        if challenge:
            cooldown = remaining_otp_cooldown_seconds(challenge)
            return render_template(
                "verify_otp.html",
                title="ยืนยันอีเมล",
                ...
                cancel_url=url_for("user_register_cancel"),
            )
        session.pop("register_otp_challenge", None)
        session.pop("register_pending", None)
...
def user_register_cancel():
    """ยกเลิกการสมัครที่ค้างอยู่ระหว่างขั้นยืนยัน OTP เพื่อให้กลับไปแก้อีเมล/ข้อมูลใหม่ได้"""
    session.pop("register_otp_challenge", None)
    session.pop("register_pending", None)
    flash("ยกเลิกรายการสมัครเดิมแล้ว กรอกข้อมูลใหม่ได้เลย", "info")
    return redirect(url_for("user_register"))
...
def recover_otp():
    ...
    return render_template(
        ...
        cancel_url=url_for("recover_cancel"),
    )
...
@app.route("/recover/cancel")
def recover_cancel():
    """ยกเลิกการกู้บัญชีที่ค้างอยู่ระหว่างขั้นยืนยัน OTP เพื่อกลับไปใส่อีเมลใหม่ได้"""
    session.pop("recover_otp_challenge", None)
    session.pop("recover_verified", None)
    session.pop("recover_user_id", None)
    flash("ยกเลิกรายการกู้บัญชีเดิมแล้ว", "info")
    return redirect(url_for("recover_account"))
...
def user_email_verify():
    ...
    return render_template(
        ...
        resend_available=True,
        cooldown_seconds=remaining_otp_cooldown_seconds(challenge),
        cancel_url=url_for("user_email_cancel"),
    )


@app.route("/account/email/cancel")
@user_login_required
def user_email_cancel():
    """ยกเลิกการเปลี่ยน/เพิ่มอีเมลที่ค้างอยู่ระหว่างขั้นยืนยัน OTP เพื่อกลับไปใส่อีเมลใหม่ได้"""
    session.pop("email_otp_challenge", None)
    session.pop("email_pending", None)
    flash("ยกเลิกรายการเดิมแล้ว ใส่อีเมลใหม่ได้เลย", "info")
    return redirect(url_for("user_email"))
