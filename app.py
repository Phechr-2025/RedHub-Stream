...
def apply_runtime_config_to_env() -> None:
    ...
    mappings = {
        ...
        "SERVICE_NAME": cfg("service_name", "SERVICE_NAME", default="menuwed"),
        "VENV_DIR": cfg("venv_dir", "VENV_DIR", default=".venv"),
...
PROJECT_NAME = os.getenv("PROJECT_NAME", meta_project_name()).strip() or meta_project_name()
APP_VERSION = os.getenv("APP_VERSION", meta_app_version()).strip() or meta_app_version()
...
app.config["SESSION_COOKIE_SECURE"] = env_flag("SESSION_COOKIE_SECURE", default=(IS_RAILWAY or PUBLIC_BASE_URL.lower().startswith("https://")))
...
def normalize_email(value: str) -> str:
    """Normalize and validate an email address for account features."""
    value = (value or "").strip().lower()
    if not value or len(value) > 254:
        return ""
    _, addr = parseaddr(value)
    if addr != value or "@" not in value or value.startswith(".") or ".." in value:
        return ""
    local, domain = value.rsplit("@", 1)
    if not local or not domain or "." not in domain or local.endswith(".") or domain.startswith(".") or domain.endswith("."):
        return ""
...
def get_otp_cooldown_seconds() -> int:
    try:
        number = int(get_setting("otp_resend_number", "1"))
        unit = get_setting("otp_resend_unit", "minutes")
        if number < 1:
            return 60
        return {
            "seconds": number,
...
def remaining_otp_cooldown_seconds(challenge) -> int:
    """คำนวณเวลาที่เหลือจริงก่อนกดส่ง OTP ใหม่ได้ โดยอิงจากเวลาที่ส่งครั้งล่าสุด
    แทนที่จะใช้ค่า cooldown เต็มจำนวนทุกครั้งที่โหลดหน้า (ป้องกันปัญหานับถอยหลังรีเซ็ตใหม่
    ทุกครั้งที่ผู้ใช้ออกจากหน้าแล้วกลับเข้ามาใหม่)"""
    cooldown = get_otp_cooldown_seconds()
    if not challenge:
        return cooldown
    last_sent = challenge["last_sent_at"] if "last_sent_at" in challenge.keys() else None
    if not last_sent:
        return cooldown
    try:
...
def otp_error_message() -> str:
    return "เกิดข้อผิดพลาดกับระบบ otp"


def brevo_ready() -> bool:
    return bool(BREVO_API_KEY and normalize_email(BREVO_SENDER_EMAIL))


def send_brevo_otp(email: str, code: str, purpose: str) -> bool:
    """Send OTP through Brevo transactional email API."""
    if not brevo_ready():
        return False
    subject_map = {
        "register": f"รหัส OTP สำหรับสมัครบัญชี {PROJECT_NAME}",
        "recover": f"รหัส OTP สำหรับกู้คืนบัญชี {PROJECT_NAME}",
        "add_email": f"รหัส OTP สำหรับเพิ่มอีเมล {PROJECT_NAME}",
        "change_email": f"รหัส OTP สำหรับเปลี่ยนอีเมล {PROJECT_NAME}",
    }
    subject = subject_map.get(purpose, f"รหัส OTP {PROJECT_NAME}")
    purpose_label_map = {
        "register": "สมัครสมาชิกใหม่",
        "recover": "กู้คืนบัญชี",
        "add_email": "เพิ่มอีเมลใหม่",
        "change_email": "เปลี่ยนอีเมล",
    }
    purpose_label = purpose_label_map.get(purpose, "ยืนยันตัวตน")
    # แสดงรหัส OTP เป็นตัวอักษรต่อเนื่องกันจริง ๆ (ไม่มีช่องว่าง ไม่มี tag แยกแต่ละหลัก)
    # เว้นระยะห่างด้วย CSS letter-spacing ล้วน ๆ เพื่อให้เวลาผู้ใช้คัดลอกรหัสไปวาง
    # จะได้ข้อความติดกันครบ 6 หลัก เช่น "699310" ไม่ใช่แยกเป็นคนละบรรทัด
    # และบังคับ white-space: nowrap เพื่อไม่ให้ตัดขึ้นบรรทัดใหม่บนจอแคบ
    html = f"""
    <html>
    <body style="margin:0;padding:0;background-color:#0f1115;font-family:'Segoe UI',Arial,sans-serif;">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#0f1115;padding:32px 16px;">
        <tr>
          <td align="center">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:440px;background-color:#1a1d24;border-radius:16px;overflow:hidden;border:1px solid #2a2e38;">
              <tr>
                <td style="background:linear-gradient(135deg,#6c5ce7,#a29bfe);padding:28px 32px;">
                  <p style="margin:0;color:#ffffff;font-size:20px;font-weight:700;letter-spacing:0.5px;">{PROJECT_NAME}</p>
                </td>
              </tr>
              <tr>
                <td style="padding:32px 32px 8px 32px;">
                  <p style="margin:0 0 4px 0;color:#8b8fa3;font-size:13px;text-transform:uppercase;letter-spacing:1px;">{purpose_label}</p>
                  <p style="margin:0 0 24px 0;color:#e6e8ef;font-size:15px;line-height:1.6;">รหัส OTP สำหรับยืนยันการทำรายการของคุณคือ</p>
                </td>
              </tr>
              <tr>
                <td style="padding:0 32px;">
                  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#0f1115;border:1px solid #2a2e38;border-radius:12px;">
                    <tr>
                      <td align="center" style="padding:20px 8px;">
                        <span style="font-size:32px;font-weight:700;letter-spacing:9px;color:#ffffff;font-family:'Courier New',monospace;white-space:nowrap;">{code}</span>
                      </td>
                    </tr>
                  </table>
                </td>
              ...
              <tr>
                <td style="padding:20px 32px 8px 32px;">
                  <p style="margin:0;color:#8b8fa3;font-size:13px;line-height:1.6;">⏱ รหัสนี้ใช้ได้ภายใน <strong style="color:#c8cbdb;">{OTP_EXPIRE_MINUTES} นาที</strong></p>
                  <p style="margin:6px 0 0 0;color:#8b8fa3;font-size:13px;line-height:1.6;">🔒 ห้ามบอกรหัสนี้ให้ผู้อื่น แม้จะอ้างว่าเป็นเจ้าหน้าที่ {PROJECT_NAME} ก็ตาม</p>
                </td>
              </tr>
              <tr>
                <td style="padding:24px 32px 28px 32px;border-top:1px solid #2a2e38;margin-top:16px;">
                  <p style="margin:16px 0 0 0;color:#565a6b;font-size:12px;line-height:1.6;">ถ้าคุณไม่ได้เป็นผู้ทำรายการนี้ สามารถละเว้นอีเมลฉบับนี้ได้เลย ไม่ต้องดำเนินการใด ๆ เพิ่มเติม</p>
                </td>
              </tr>
            </table>
            <p style="margin:20px 0 0 0;color:#565a6b;font-size:11px;">© {PROJECT_NAME} · อีเมลนี้ส่งโดยอัตโนมัติ กรุณาอย่าตอบกลับ</p>
          </td>
        </tr>
      </table>
    </body></html>
    ...
    payload = {
        ...
        "subject": subject,
        "htmlContent": html,
        "textContent": f"รหัส OTP ของคุณคือ {code} ใช้ได้ภายใน {OTP_EXPIRE_MINUTES} นาที",
        "tags": ["otp", purpose, PROJECT_NAME],
    }
    try:
...
def cleanup_otp_challenges(conn: sqlite3.Connection) -> None:
...
def create_otp_challenge(
    ...
    *,
    ...
    """Create and send an OTP. Returns (challenge_id, cooldown_seconds)."""
    email = normalize_email(email)
    if not email:
        return None, 0
    cleanup_otp_challenges(conn)
    now = datetime.utcnow()
    cooldown = get_otp_cooldown_seconds()
    existing = conn.execute(
        """
        SELECT id, last_sent_at, verified_at
        FROM otp_challenges
        WHERE purpose = ? AND lower(email) = ? AND verified_at IS NULL
    ...
    if not send_brevo_otp(email, code, purpose):
        return None, -1
    ...
    if existing:
        conn.execute(
            ...
            SET code_hash = ?, expires_at = ?, attempts = 0, last_sent_at = ?, verified_at = NULL,
                user_id = ?, challenge_id = ?
            WHERE id = ?
            """,
            (code_hash, expires, sent_at, user_id, challenge_id, existing["id"]),
        )
    else:
        conn.execute(
            """
            INSERT INTO otp_challenges
            (challenge_id, purpose, email, user_id, code_hash, expires_at, attempts, last_sent_at)
            VALUES (?, ?, ?, ?, ?, ?, 0, ?)
            """,
            (challenge_id, purpose, email, user_id, code_hash, expires, sent_at),
        )
    conn.commit()
    return challenge_id, cooldown


def get_session_otp(conn: sqlite3.Connection, flow_key: str):
    challenge_id = session.get(flow_key)
    if not challenge_id:
        return None
    cleanup_otp_challenges(conn)
    return conn.execute(
        "SELECT * FROM otp_challenges WHERE challenge_id = ? AND verified_at IS NULL",
        (challenge_id,),
    ).fetchone()


def verify_session_otp(conn: sqlite3.Connection, flow_key: str, code: str):
    row = get_session_otp(conn, flow_key)
    if row is None:
        return None, "ไม่มีคำขอ OTP ที่ใช้งานอยู่ หรือ OTP หมดอายุแล้ว"
    try:
        if datetime.utcnow() >= datetime.fromisoformat(row["expires_at"]):
            return None, "OTP หมดอายุแล้ว กรุณาส่ง OTP ใหม่"
    except Exception:
        return None, "OTP หมดอายุแล้ว กรุณาส่ง OTP ใหม่"
    if int(row["attempts"] or 0) >= OTP_MAX_ATTEMPTS:
        return None, "ใส่ OTP ผิดเกินจำนวนที่กำหนด กรุณาส่ง OTP ใหม่"
    code = (code or "").strip()
...
def email_account_limit(email: str, exclude_user_id: int | None = None) -> int:
    try:
        limit = int(get_setting("email_accounts_limit", "1"))
    except Exception:
        limit = 1
    return max(1, limit)


def email_usage_count(conn: sqlite3.Connection, email: str, exclude_user_id: int | None = None) -> int:
    email = normalize_email(email)
    if not email:
        return 0
    if exclude_user_id is None:
        row = conn.execute(
            "SELECT COUNT(*) AS c FROM users WHERE lower(email) = ?", (email,)
        ).fetchone()
    else:
        row = conn.execute(
            "SELECT COUNT(*) AS c FROM users WHERE lower(email) = ? AND id != ?",
            (email, exclude_user_id),
        ).fetchone()
    return int(row["c"] if row else 0)


def ensure_otp_tables(conn: sqlite3.Connection):
    conn.execute(
        """
...
def ensure_user_email_column(conn: sqlite3.Connection):
...
def ensure_backup_history_table(conn: sqlite3.Connection):
    conn.execute(
        """
...
def init_db():
    ...
    ensure_user_email_column(conn)
    ensure_password_history_table(conn)
    ensure_settings_table(conn)
    ensure_otp_tables(conn)
    ensure_backup_history_table(conn)
...
# routes ที่ถือว่ายังอยู่ "ในขั้นตอนเดียวกัน" ของแต่ละ flow OTP
# ถ้าผู้ใช้ไปที่ route อื่นนอกเหนือจากนี้ระหว่างที่มีรายการ OTP ค้างอยู่ ถือว่ายกเลิกรายการนั้นทันที
REGISTER_FLOW_ENDPOINTS = {
    "user_register", "user_register_otp", "user_register_verify", "user_register_resend_otp",
}
RECOVER_FLOW_ENDPOINTS = {
    "recover_account", "recover_otp", "recover_resend_otp", "recover_reset",
}
EMAIL_FLOW_ENDPOINTS = {
    "user_email", "user_email_verify", "user_email_resend_otp",
}


@app.before_request
def auto_cancel_abandoned_otp_flows():
    """ถ้าผู้ใช้กำลังอยู่กลางขั้นตอนยืนยัน OTP (สมัครสมาชิก / กู้บัญชี / เปลี่ยนอีเมล) แล้วออกไปหน้าอื่น
    ที่ไม่ใช่ส่วนหนึ่งของ flow นั้น (เช่น กดเมนู "เข้าสู่ระบบ" หรือหน้าอื่นใด ๆ) ให้ถือว่ายกเลิกรายการนั้น
    ทันที เพื่อไม่ให้ค้างที่หน้า OTP ตลอดไป และเมื่อกลับมาที่ flow เดิมจะต้องกรอกข้อมูลใหม่ตั้งแต่ต้น"""
    endpoint = request.endpoint
    if endpoint is None or endpoint == "static":
        return

    if session.get("register_otp_challenge") and endpoint not in REGISTER_FLOW_ENDPOINTS:
        session.pop("register_otp_challenge", None)
        session.pop("register_pending", None)

    if session.get("recover_otp_challenge") and endpoint not in RECOVER_FLOW_ENDPOINTS:
        session.pop("recover_otp_challenge", None)

    if (session.get("recover_verified") or session.get("recover_user_id")) and endpoint not in RECOVER_FLOW_ENDPOINTS:
        session.pop("recover_verified", None)
        session.pop("recover_user_id", None)

    if session.get("email_otp_challenge") and endpoint not in EMAIL_FLOW_ENDPOINTS:
        session.pop("email_otp_challenge", None)
...
def watch_episode(series_id, episode_id):
    ...
    episode_file_path = episode["file_path"] if "file_path" in episode.keys() else None
    episode_drive_id = episode["drive_id"] if "drive_id" in episode.keys() else None

    media_source = episode_file_path or raw_video_url
    if not media_source and episode_drive_id:
        media_source = f"{episode_drive_id}.mp4"

    media_mime = guess_mime_type(media_source)
    media_kind = media_kind_from_mime(media_mime)
...
def stream_episode(episode_id):
    ...
    stream_source = abs_path or file_path or video_url
    stream_mime = guess_mime_type(stream_source)
    ...
    if source_type == "gdrive" and drive_id:
        try:
            ...
            response = send_file(
                new_file,
                mimetype=guess_mime_type(new_file),
                as_attachment=False,
                conditional=True,
                etag=True,
...
def user_register():
    ...
    # ไม่ให้ค้างที่หน้า OTP หรือมีข้อมูลเดิมค้างอยู่
    if request.method == "GET" and session.get("register_otp_challenge"):
        session.pop("register_otp_challenge", None)
        session.pop("register_pending", None)
    ...
    if request.method == "POST":
        ...
        if not verify_turnstile(token, get_client_ip()):
            form_error = "กรุณายืนยันว่าท่านไม่ใช่บอทก่อนสมัครสมาชิก"
        ...
        if not username or not password or not password_confirm:
            form_error = "กรุณากรอกข้อมูลให้ครบ"
        elif force_email and not email:
            form_error = "กรุณาใส่อีเมล์ของคุณ"
        elif email_raw and not email:
            form_error = "กรุณาใส่อีเมล์ให้ถูกต้อง"
        elif password != password_confirm:
            form_error = "รหัสผ่านใหม่และยืนยันรหัสผ่านไม่ตรงกัน"
        else:
            conn = get_db_connection()
            try:
                ...
                if existing_username:
                    form_error = "ชื่อผู้ใช้นี้มีอยู่ในระบบแล้ว"
                elif email:
                    limit = email_account_limit(email)
                    if email_usage_count(conn, email) >= limit:
                        form_error = "อีเมล์นี้ถูกใช้ครบจำนวนบัญชีที่กำหนดแล้ว"
                if form_error:
                    flash(form_error, "error")
                elif force_email:
                    challenge_id, cooldown = create_otp_challenge(
                        conn, purpose="register", email=email
                    )
                    if challenge_id is None:
                        if cooldown == -1:
                            form_error = otp_error_message()
                        else:
                            form_error = f"กรุณารอ {format_duration_label(cooldown)} ก่อนส่ง OTP อีกครั้ง"
                        flash(form_error, "error")
                    else:
                        session["register_otp_challenge"] = challenge_id
                        ...
                        conn.close()
                        return redirect(url_for("user_register_otp"))
                else:
                    hashed = generate_password_hash(password)
                    user_key = generate_user_key()
                    conn.execute(
                        "INSERT INTO users (username, password, plain_password, user_key, email, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                        (
                            username,
                            hashed,
                            password,
...
@app.route("/register/otp", methods=["GET"])
def user_register_otp():
    """หน้ายืนยัน OTP ของขั้นตอนสมัครสมาชิก แยก URL ออกจาก /register โดยเฉพาะ
    เพื่อให้ปุ่มย้อนกลับของเบราว์เซอร์พาไปหน้าฟอร์มสมัครที่ว่างเปล่าได้จริง ๆ"""
    if session.get("user_id"):
        return redirect(url_for("my_page"))

    pending = session.get("register_pending") or {}
    if not pending or not session.get("register_otp_challenge"):
        return redirect(url_for("user_register"))

    conn = get_db_connection()
    challenge = get_session_otp(conn, "register_otp_challenge")
    conn.close()
    if not challenge:
        session.pop("register_otp_challenge", None)
        session.pop("register_pending", None)
        flash("OTP หมดอายุแล้ว กรุณาสมัครใหม่อีกครั้ง", "error")
        return redirect(url_for("user_register"))
    ...
    return no_store(render_template(
        ...
        description=f"กรุณากรอก OTP ที่ส่งไปยัง {challenge['email']}",
        flow="register",
        resend_available=True,
        cooldown_seconds=remaining_otp_cooldown_seconds(challenge),
    ))
...
def user_register_verify():
    pending = session.get("register_pending") or {}
    if not pending or not session.get("register_otp_challenge"):
        flash("ไม่พบรายการสมัครที่รอยืนยัน OTP", "error")
        return redirect(url_for("user_register"))
    ...
    if not verify_turnstile(token, get_client_ip()):
        conn.close()
        flash("กรุณายืนยันว่าท่านไม่ใช่บอทก่อนยืนยัน OTP", "error")
        return redirect(url_for("user_register_otp"))

    challenge, error = verify_session_otp(conn, "register_otp_challenge", code)
    if error:
        conn.close()
        flash(error, "error")
        return redirect(url_for("user_register_otp"))
    ...
    try:
        ...
        if not username or not password or not email:
            raise ValueError("ข้อมูลสมัครสมาชิกไม่สมบูรณ์")
        if conn.execute("SELECT id FROM users WHERE username = ?", (username,)).fetchone():
            raise ValueError("ชื่อผู้ใช้นี้มีอยู่ในระบบแล้ว")
        if email_usage_count(conn, email) >= email_account_limit(email):
            raise ValueError("อีเมล์นี้ถูกใช้ครบจำนวนบัญชีที่กำหนดแล้ว")
        conn.execute(
            "INSERT INTO users (username, password, plain_password, user_key, email, created_at) VALUES (?, ?, ?, ?, ?, ?)",
            (
                username,
    ...
    session.pop("register_otp_challenge", None)
    session.pop("register_pending", None)
    flash("ยืนยันอีเมลและสมัครบัญชีสำเร็จ กรุณาเข้าสู่ระบบ", "success")
    return redirect(url_for("user_login"))


@app.route("/register/resend-otp", methods=["POST"])
def user_register_resend_otp():
    pending = session.get("register_pending") or {}
    if not pending or not session.get("register_otp_challenge"):
        flash("ไม่พบรายการ OTP ที่ต้องส่งซ้ำ", "error")
        return redirect(url_for("user_register"))
    email = normalize_email(pending.get("email", ""))
    conn = get_db_connection()
    challenge_id, wait = create_otp_challenge(conn, purpose="register", email=email)
    if challenge_id is None:
        conn.close()
        flash(
            otp_error_message() if wait == -1
            else f"กรุณารอ {format_duration_label(wait)} ก่อนส่ง OTP อีกครั้ง",
            "error",
        )
        return redirect(url_for("user_register_otp"))
    session["register_otp_challenge"] = challenge_id
    conn.close()
    flash("ส่ง OTP ใหม่แล้ว", "success")
    return redirect(url_for("user_register_otp"))
...
def user_login():
    ...
    if request.method == "POST":
        token = request.form.get("cf-turnstile-response")
        if not verify_turnstile(token, get_client_ip()):
            form_error = "กรุณายืนยันว่าท่านไม่ใช่บอทก่อนเข้าสู่ระบบ"
            flash(form_error, "error")
            return render_template("user_login.html", form_error=form_error)
        ...
        if user is None:
            form_error = "ไม่พบบัญชีผู้ใช้นี้"
            flash(form_error, "error")
        elif not check_password_hash(user["password"], password):
            form_error = "รหัสผ่านไม่ถูกต้อง"
            flash(form_error, "error")
        else:
            login_user(user)
...
def recover_account():
    ...
    # ถ้ามีรายการกู้บัญชีค้างรอ OTP อยู่แต่ผู้ใช้กลับมาที่หน้านี้เอง (เช่น กดปุ่มย้อนกลับจากหน้า
    # ยืนยัน OTP) ให้ยกเลิกรายการเดิมทิ้งไปเงียบ ๆ แล้วโชว์ฟอร์มเปล่าให้กรอกใหม่
    if request.method == "GET" and session.get("recover_otp_challenge"):
        session.pop("recover_otp_challenge", None)

    if request.method == "POST":
        token = request.form.get("cf-turnstile-response")
        if not verify_turnstile(token, get_client_ip()):
            flash("กรุณายืนยันว่าท่านไม่ใช่บอทก่อนส่ง OTP", "error")
            return no_store(render_template("recover_account.html"))

        email = normalize_email(request.form.get("email", ""))
        if not email:
            flash("กรุณาใส่อีเมล์ของคุณ", "error")
            return no_store(render_template("recover_account.html"))
        conn = get_db_connection()
        user_count = email_usage_count(conn, email)
        if user_count < 1:
            conn.close()
            flash("ไม่พบบัญชีที่ใช้อีเมล์นี้", "error")
            return no_store(render_template("recover_account.html"))
        # การกู้คืนต้องระบุบัญชีเดียวเพื่อป้องกันการเปลี่ยนรหัสผ่านผิดบัญชี
        users = conn.execute(
            "SELECT id, username FROM users WHERE lower(email) = ? ORDER BY id ASC",
            (email,),
        ).fetchall()
        if len(users) != 1:
            conn.close()
            flash("อีเมล์นี้ผูกกับหลายบัญชี ไม่สามารถกู้คืนอัตโนมัติได้ กรุณาติดต่อแอดมิน", "error")
            return no_store(render_template("recover_account.html"))
        challenge_id, wait = create_otp_challenge(
            conn, purpose="recover", email=email, user_id=users[0]["id"]
        )
        conn.close()
        if challenge_id is None:
            flash(
            ...
            return no_store(render_template("recover_account.html"))
        session["recover_otp_challenge"] = challenge_id
        return redirect(url_for("recover_otp"))
...
def recover_otp():
    ...
    if not enabled:
        return redirect(url_for("user_login"))
    conn = get_db_connection()
    challenge = get_session_otp(conn, "recover_otp_challenge")
    conn.close()
    if not challenge:
        flash("ไม่พบรายการ OTP กรุณาเริ่มกู้บัญชีใหม่", "error")
    ...
    if request.method == "POST":
        ...
        conn = get_db_connection()
        verified, error = verify_session_otp(conn, "recover_otp_challenge", request.form.get("otp", ""))
        if error:
            conn.close()
            flash(error, "error")
            return redirect(url_for("recover_otp"))
        session["recover_verified"] = True
        session["recover_user_id"] = verified["user_id"]
        conn.close()
        return redirect(url_for("recover_reset"))

    return no_store(render_template(
        "verify_otp.html",
        title="ยืนยัน OTP กู้คืนบัญชี",
        heading="ยืนยัน OTP กู้คืนบัญชี",
        description=f"กรุณากรอก OTP ที่ส่งไปยัง {challenge['email']}",
        flow="recover",
        resend_available=True,
        cooldown_seconds=remaining_otp_cooldown_seconds(challenge),
...
@app.route("/recover/resend-otp", methods=["POST"])
def recover_resend_otp():
    if not session.get("recover_otp_challenge"):
        return redirect(url_for("recover_account"))
    conn = get_db_connection()
    old = get_session_otp(conn, "recover_otp_challenge")
    email = old["email"] if old else ""
    user_id = old["user_id"] if old else None
    challenge_id, wait = create_otp_challenge(
        conn, purpose="recover", email=email, user_id=user_id
    )
    conn.close()
    if challenge_id is None:
        flash(
...
def recover_reset():
    if not session.get("recover_verified") or not session.get("recover_user_id"):
        return redirect(url_for("recover_account"))
    if request.method == "POST":
        password = request.form.get("password", "")
        confirm = request.form.get("password_confirm", "")
        if not password or not confirm:
            flash("กรุณากรอกรหัสผ่านให้ครบ", "error")
        elif password != confirm:
            flash("รหัสผ่านใหม่และยืนยันรหัสผ่านไม่ตรงกัน", "error")
        else:
            conn = get_db_connection()
            user = conn.execute("SELECT * FROM users WHERE id = ?", (session["recover_user_id"],)).fetchone()
            if not user:
                conn.close()
            ...
            conn.execute(
                ...
                (generate_password_hash(password), password, user["id"]),
            )
            ensure_password_history_table(conn)
            conn.execute(
                "INSERT INTO password_history (user_id, old_password, new_password, changed_at) VALUES (?, ?, ?, ?)",
                (user["id"], old_plain, password, datetime.utcnow().isoformat()),
            )
            conn.commit()
            conn.close()
            session.pop("recover_otp_challenge", None)
...
def user_account():
    ...
    if request.method == "POST":
        ...
        if action == "change_password":
            ...
            conn = get_db_connection()
            new_key = generate_user_key()
            conn.execute("UPDATE users SET user_key = ? WHERE id = ?", (new_key, user["id"]))
            conn.commit()
            conn.close()
...
def user_email():
    ...
    if request.method == "GET" and session.get("email_otp_challenge"):
        session.pop("email_otp_challenge", None)
        session.pop("email_pending", None)
    ...
    if request.method == "POST":
        email = normalize_email(request.form.get("email", ""))
        if not email:
            flash("กรุณาใส่อีเมล์ให้ถูกต้อง", "error")
        elif has_email and not email_change_enabled:
            flash("เปิดการบริการเปลี่ยนอีเมล์ชั่วคราว ขออภัยในความไม่สะดวก", "error")
        else:
            conn = get_db_connection()
            limit = email_account_limit(email)
            if email_usage_count(conn, email, exclude_user_id=user["id"]) >= limit:
                conn.close()
                flash("อีเมล์นี้ถูกใช้ครบจำนวนบัญชีที่กำหนดแล้ว", "error")
            else:
                purpose = "change_email" if has_email else "add_email"
                challenge_id, wait = create_otp_challenge(
                    conn, purpose=purpose, email=email, user_id=user["id"]
                )
                conn.close()
                if challenge_id is None:
                    flash(
                    ...
                    session["email_otp_challenge"] = challenge_id
                    session["email_pending"] = {"email": email, "user_id": user["id"]}
                    return redirect(url_for("user_email_verify"))

    return no_store(render_template(
        "user_email.html",
        user=user,
        has_email=has_email,
        email_change_enabled=email_change_enabled,
    ))


@app.route("/account/email/verify", methods=["GET", "POST"])
@user_login_required
def user_email_verify():
    ...
    pending = session.get("email_pending") or {}
    conn = get_db_connection()
    challenge = get_session_otp(conn, "email_otp_challenge")
    conn.close()
    if not challenge or int(challenge["user_id"] or 0) != user["id"]:
        flash("ไม่พบรายการ OTP กรุณาเริ่มใหม่", "error")
    ...
    if request.method == "POST":
        ...
        conn = get_db_connection()
        verified, error = verify_session_otp(conn, "email_otp_challenge", request.form.get("otp", ""))
        if error:
            conn.close()
            flash(error, "error")
            return redirect(url_for("user_email_verify"))
        ...
        session.pop("email_otp_challenge", None)
        session.pop("email_pending", None)
        flash("เปลี่ยนอีเมล์เรียบร้อย", "success")
        return redirect(url_for("user_account"))
...
@app.route("/account/email/resend-otp", methods=["POST"])
@user_login_required
def user_email_resend_otp():
    user = get_current_user()
    if user is None:
        return redirect(url_for("user_login"))
    pending = session.get("email_pending") or {}
    ...
    if not email:
        return redirect(url_for("user_email"))
    purpose = "change_email" if (user["email"] or "").strip() else "add_email"
    conn = get_db_connection()
    challenge_id, wait = create_otp_challenge(
        conn, purpose=purpose, email=email, user_id=user["id"]
    )
...
def admin_login():
    ...
    if request.method == "POST":
        token = request.form.get("cf-turnstile-response")
        if not verify_turnstile(token, get_client_ip()):
            form_error = "กรุณายืนยันว่าท่านไม่ใช่บอทก่อนเข้าสู่ระบบแอดมิน"
            flash(form_error, "error")
        ...
        if username == current_admin_username and password == current_admin_password:
            ...
            form_error = "ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง"
            flash(form_error, "error")
...
def admin_account():
    ...
    if request.method == "POST":
        ...
        if not new_username or not new_password:
            ...
            config = load_menuwed_config()
            config["admin_username"] = new_username
            config["admin_password"] = new_password
            save_menuwed_config(config)
            os.environ["ADMIN_USERNAME"] = new_username
            os.environ["ADMIN_PASSWORD"] = new_password
            flash("บันทึกชื่อผู้ใช้และรหัสผ่านแอดมินลง config แล้ว", "success")
...
def admin_user_detail(user_id):
    ...
    if request.method == "POST":
        ...
        if action == "update_account":
            ...
            new_email_raw = request.form.get("email", "").strip()
            new_email = normalize_email(new_email_raw) if new_email_raw else None

            if not new_username:
                flash("กรุณากรอกชื่อผู้ใช้", "error")
            elif new_email_raw and not new_email:
                flash("รูปแบบอีเมล์ไม่ถูกต้อง", "error")
            elif new_email and email_usage_count(conn, new_email, exclude_user_id=user_id) >= email_account_limit(new_email):
                flash("อีเมล์นี้ถูกใช้ครบจำนวนบัญชีที่กำหนดแล้ว", "error")
            else:
                try:
                    if new_password:
                        old_plain = user["plain_password"] if user["plain_password"] else None
                        conn.execute(
                            "UPDATE users SET username = ?, password = ?, plain_password = ?, email = ? WHERE id = ?",
                            (new_username, generate_password_hash(new_password), new_password, new_email, user_id),
                        )
                        ensure_password_history_table(conn)
                        conn.execute(
                            "INSERT INTO password_history (user_id, old_password, new_password, changed_at) VALUES (?, ?, ?, ?)",
            ...
            new_key = generate_user_key()
            conn.execute("UPDATE users SET user_key = ? WHERE id = ?", (new_key, user_id))
            conn.commit()
...
def admin_settings():
    ...
    if request.method == "POST":
        ...
        if not user_num.isdigit() or int(user_num) < 1 or user_unit not in {"minutes", "hours", "days"}:
            flash("กรุณากรอกระยะเวลา session ผู้ใช้ให้ถูกต้อง", "error")
            has_error = True
        else:
        ...
        if not admin_num.isdigit() or int(admin_num) < 1 or admin_unit not in {"minutes", "hours", "days"}:
            flash("กรุณากรอกระยะเวลา session แอดมินให้ถูกต้อง", "error")
            has_error = True
        else:
        ...
        # Email / OTP settings
        force_email = "true" if request.form.get("force_email") == "on" else "false"
        recovery_enabled = "true" if request.form.get("account_recovery_enabled") == "on" else "false"
        email_change_enabled = "true" if request.form.get("email_change_enabled") == "on" else "false"
        set_setting("force_email", force_email)
        set_setting("account_recovery_enabled", recovery_enabled)
        set_setting("email_change_enabled", email_change_enabled)

        resend_num = request.form.get("otp_resend_number", "1").strip()
        resend_unit = request.form.get("otp_resend_unit", "minutes").strip()
        if not resend_num.isdigit() or int(resend_num) < 1 or resend_unit not in {"seconds", "minutes", "hours"}:
            flash("กรุณากำหนดเวลาส่ง OTP ซ้ำให้ถูกต้อง", "error")
            has_error = True
        else:
            set_setting("otp_resend_number", resend_num)
            set_setting("otp_resend_unit", resend_unit)

        email_limit = request.form.get("email_accounts_limit", "1").strip()
        if not email_limit.isdigit() or int(email_limit) < 1:
            flash("จำนวนอีเมล์ต่อ 1 บัญชีต้องเป็นตัวเลขตั้งแต่ 1 ขึ้นไป", "error")
            has_error = True
        else:
            set_setting("email_accounts_limit", email_limit)
...
# ---------- ระบบสำรอง/คืนค่า ----------
def current_backup_payload(conn: sqlite3.Connection, backup_type: str) -> dict:
    if backup_type == "users":
        ensure_user_extra_columns(conn)
        ensure_password_history_table(conn)
...
def save_latest_backup(conn: sqlite3.Connection, backup_type: str, operation: str) -> str:
    root = Path(DATA_DIR) / "backups"
    root.mkdir(parents=True, exist_ok=True)
    path = root / f"latest_{backup_type}.json"
    payload = current_backup_payload(conn, backup_type)
    path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    ensure_backup_history_table(conn)
    conn.execute(
        "INSERT INTO backup_history (operation, backup_type, file_path, created_at) VALUES (?, ?, ?, ?)",
        (operation, backup_type, str(path), datetime.utcnow().isoformat()),
    )
    conn.commit()
    return str(path)


def latest_backup_info(conn: sqlite3.Connection):
    ensure_backup_history_table(conn)
    rows = conn.execute(
...
def admin_backup():
    ...
    if request.method == "POST":
        ...
        try:
            ensure_backup_history_table(conn)
            if mode == "replace" and backup_type in ("videos", "users"):
                save_latest_backup(conn, backup_type, "before_restore")
            ...
            if backup_type == "videos":
                ...
                for u in users_list:
                    ...
                    email = normalize_email(u.get("email", "")) or None
                    ...
                    if existing:
                        cur.execute(
                            """
                            UPDATE users
                            SET username = ?, password = ?, plain_password = ?, user_key = ?, email = ?, created_at = ?
                            WHERE id = ?
                            """,
                            (
                                username,
                        ...
                        cur.execute(
                            """
                            INSERT INTO users (id, username, password, plain_password, user_key, email, created_at)
                            VALUES (?, ?, ?, ?, ?, ?, ?)
                            """,
                            (
                                uid,
    ...
    conn = get_db_connection()
    latest_history = latest_backup_info(conn)
    conn.close()
    return render_template("admin_backup.html", latest_history=latest_history)
...
def admin_backup_download_videos():
    ...
    ensure_backup_history_table(conn)
    try:
        save_latest_backup(conn, "videos", "download")
    finally:
        conn.close()
...
def admin_backup_download_users():
    ...
    ensure_backup_history_table(conn)
    try:
        save_latest_backup(conn, "users", "download")
    finally:
        conn.close()
