...
cat > "$HOME/.local/bin/menuwed" <<'EOF'
#!/usr/bin/env sh
set -eu
exec "${PYTHON:-python3}" "__INSTALL_PATH__/menuwed.py" "$@"
EOF
sed -i "s#__INSTALL_PATH__#$INSTALL_PATH#g" "$HOME/.local/bin/menuwed"
chmod 755 "$HOME/.local/bin/menuwed"

if command -v sudo >/dev/null 2>&1; then
  sudo ln -sf "$HOME/.local/bin/menuwed" /usr/local/bin/menuwed 2>/dev/null || true
fi
...
def private_host() -> str:
    ...
    try:
        ...
        return "127.0.0.1"
    return host if (addr.is_private or addr.is_loopback or addr.is_link_local) else "127.0.0.1"

def public_host(port: int, env: dict[str, str]) -> str:
...
try:
    pid_alive = pid != "-" and int(pid) > 0
    if pid_alive:
        os.kill(int(pid), 0)
except Exception:
    pid_alive = False
status = "กำลังทำงาน" if pid_alive else "หยุดอยู่"
private_url = f"http://{private_host()}:{port}"
public_url = public_host(port, env)
