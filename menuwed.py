...
def web_status() -> dict[str, str]:
    ...
    return {
        ...
        "firewall": firewall_status_best_effort(port),
        "port": str(port),
...
def bind_domain() -> None:
    ...
    if value:
        ...
        if port == 5000:
            print("หมายเหตุ: ถ้าใช้ Cloudflare แบบ Proxy และยังขึ้น 522 อาจต้องมี reverse proxy บนพอร์ต 80/443 หรือใช้ DNS only")
    else:
        print("ล้างโดเมนแล้ว")
