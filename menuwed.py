...
def print_web_status() -> None:
    ...
    if is_private_access_host(status['url'].split('://', 1)[-1].split(':', 1)[0]):
        print("หมายเหตุ: ที่อยู่นี้เป็น private IP ของเครื่อง ไม่ใช่ public URL")
...
def show_web_info() -> None:
    ...
    url = public_url()
    print(f"URL เข้าชม: {url}")
    if "PUBLIC_BASE_URL" not in os.environ and is_private_access_host(url.split('://', 1)[-1].split(':', 1)[0]):
        print("หมายเหตุ: URL นี้เป็น IP ภายในเครื่อง/VPC ใช้จากอินเทอร์เน็ตตรง ๆ ไม่ได้ ต้องใช้ public IP หรือโดเมน")
