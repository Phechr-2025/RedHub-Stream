...
def print_web_status() -> None:
    ...
    private_value = private_url(port)
    public_value = public_url(port)
    private_host = private_value.split("://", 1)[-1].split(":", 1)[0]
    print("\n=== สถานะเว็บ ===")
    ...
    print(f"Private URL: {private_value}")
    print(f"Public URL: {public_value}")
    if private_host == "127.0.0.1" and not read_env_file(env_path()).get("PUBLIC_BASE_URL"):
...
def show_web_info() -> None:
    ...
    status = web_status()
    private_url_value = private_url(port)
    public_url_value = public_url(port)
    ...
    print(f"🌐 พอร์ต : {port}")
    print("🔥 Firewall : ไม่ทราบ")
    print(f"🟢 สถานะ : {'กำลังทำงาน' if status['alive'] == 'yes' else 'หยุดอยู่'}")
    print(f"🆔 PID : {status['pid']}")
    print(f"🔗 Private URL : {private_url_value}")
