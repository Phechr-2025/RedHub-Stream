...
# nginx/reverse proxy paths used when binding a public domain
NGINX_SITE_AVAILABLE = Path("/etc/nginx/sites-available") / f"{SCRIPT_NAME}.conf"
NGINX_SITE_ENABLED = Path("/etc/nginx/sites-enabled") / f"{SCRIPT_NAME}.conf"
NGINX_SSL_DIR = Path("/etc/ssl/menuwed")
CERTBOT_RENEWAL_HOOKS_DIR = Path("/etc/letsencrypt/renewal-hooks")
CERTBOT_PRE_HOOK = CERTBOT_RENEWAL_HOOKS_DIR / "pre" / f"{SCRIPT_NAME}-stop-nginx.sh"
CERTBOT_DEPLOY_HOOK = CERTBOT_RENEWAL_HOOKS_DIR / "deploy" / f"{SCRIPT_NAME}-reload-nginx.sh"
CERTBOT_POST_HOOK = CERTBOT_RENEWAL_HOOKS_DIR / "post" / f"{SCRIPT_NAME}-start-nginx.sh"
CERTBOT_RENEW_CRON = Path("/etc/cron.d") / f"{SCRIPT_NAME}-certbot-renew"
...
def ensure_env(force: bool = False) -> Path:
    ...
    if target.exists() and not force:
        ...
        merged["BREVO_API_KEY"] = merged.get("BREVO_API_KEY") or ""
        merged["BREVO_SENDER_EMAIL"] = merged.get("BREVO_SENDER_EMAIL") or ""
        merged["BREVO_SENDER_NAME"] = merged.get("BREVO_SENDER_NAME") or config_project_name()
...
def reverse_proxy_enabled() -> bool:
    if not web_public_domain():
        return False
    return NGINX_SITE_ENABLED.exists() or NGINX_SITE_ENABLED.is_symlink() or NGINX_SITE_AVAILABLE.exists()
...
def check_public_web_reachable(url: str, timeout: float = 4.0) -> bool:
    ...
    for candidate in candidates:
        ...
        req = urllib.request.Request(f"{candidate}/healthz", headers={"User-Agent": SCRIPT_NAME})
        try:
...
def _ensure_certbot_available() -> bool:
    if shutil.which("certbot"):
        return True

    if shutil.which("sudo") and shutil.which("apt-get"):
        subprocess.run(["sudo", "-n", "apt-get", "update"], capture_output=True, text=True)
        subprocess.run(["sudo", "-n", "apt-get", "install", "-y", "certbot"], capture_output=True, text=True)

    return shutil.which("certbot") is not None


def _write_certbot_hook(target: Path, command_line: str) -> bool:
    content = "#!/bin/sh\nset -eu\n" + command_line.rstrip() + "\n"
    try:
        _write_privileged_text(target, content, mode=0o755)
        return True
    except Exception:
        return False


def _certbot_hook_command(action: str) -> str:
    return (
        'if command -v systemctl >/dev/null 2>&1; then\n'
        f'    systemctl {action} nginx >/dev/null 2>&1 || true\n'
        'fi\n'
        'exit 0'
    )


def _ensure_certbot_auto_renew(domain: str) -> str:
    if not _ensure_certbot_available():
        return "ไม่พบ certbot สำหรับตั้ง auto renew"

    hook_ok = True
    hook_ok = _write_certbot_hook(CERTBOT_PRE_HOOK, _certbot_hook_command("stop")) and hook_ok
    hook_ok = _write_certbot_hook(CERTBOT_DEPLOY_HOOK, _certbot_hook_command("restart")) and hook_ok
    hook_ok = _write_certbot_hook(CERTBOT_POST_HOOK, _certbot_hook_command("start")) and hook_ok

    timer_enabled = False
    systemctl = shutil.which("systemctl")
    sudo = shutil.which("sudo") if not is_root_user() else None
    if systemctl:
        timer_cmd = ["systemctl", "enable", "--now", "certbot.timer"]
        if not is_root_user():
            if sudo:
                timer_cmd = [sudo, "-n", *timer_cmd]
            else:
                timer_cmd = []
        if timer_cmd:
            proc = subprocess.run(timer_cmd, capture_output=True, text=True)
            timer_enabled = proc.returncode == 0

    if not timer_enabled:
        cron_content = (
            "SHELL=/bin/sh\n"
            "PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin\n"
            f"17 3 * * * root certbot renew --quiet >/dev/null 2>&1\n"
        )
        try:
            _write_privileged_text(CERTBOT_RENEW_CRON, cron_content, mode=0o644)
            timer_enabled = True
        except Exception:
            pass

    parts = ["เปิด auto renew แล้ว"]
    if hook_ok:
        parts.append("ตั้งค่า renewal hooks แล้ว")
    else:
        parts.append("ตั้งค่า renewal hooks ไม่ครบ")
    parts.append("certbot.timer" if systemctl and timer_enabled else "cron fallback")
    return " ; ".join(parts)


def _is_letsencrypt_certificate(cert_path: Path, key_path: Path) -> bool:
    cert_text = str(cert_path).replace("\\", "/")
    key_text = str(key_path).replace("\\", "/")
    return cert_text.startswith("/etc/letsencrypt/live/") and key_text.startswith("/etc/letsencrypt/live/")



def _letsencrypt_cert_ready(domain: str) -> bool:
    cert_path, key_path = _letsencrypt_cert_paths(domain)
    if is_root_user():
        try:
    ...
    for target in (cert_path, key_path):
        proc = subprocess.run([sudo, "-n", "test", "-r", str(target)], capture_output=True, text=True)
        if proc.returncode != 0:
            return False
    return True


def _issue_letsencrypt_cert(domain: str) -> tuple[Path, Path] | None:
    cert_path, key_path = _letsencrypt_cert_paths(domain)

    if _letsencrypt_cert_ready(domain):
        _ensure_certbot_auto_renew(domain)
    ...
    nginx_was_active = _nginx_service_active()
    try:
        if nginx_was_active:
            try:
                _run_privileged(["systemctl", "stop", "nginx"], capture_output=True, text=True)
            except Exception:
                pass
        ...
        if nginx_was_active:
            try:
                _run_privileged(["systemctl", "start", "nginx"], capture_output=True, text=True)
            except Exception:
                pass
...
def ensure_reverse_proxy(domain: str | None = None) -> str:
    ...
    try:
        cert_path, key_path = _issue_letsencrypt_cert(domain)
        if cert_path is None or key_path is None:
            return (
                "สร้าง certificate ไม่สำเร็จ: ใช้ได้เฉพาะ Let’s Encrypt และระบบยังออกใบรับรองไม่ได้ "
                "(ตรวจ DNS, port 80, sudo/NOPASSWD และ Cloudflare proxy)"
            )
    except Exception as exc:
        return f"สร้าง certificate ไม่สำเร็จ: {exc}"

    site_text = _render_nginx_site(domain, port, cert_path, key_path)
    auto_renew_msg = "ใช้ Let’s Encrypt พร้อมต่ออายุอัตโนมัติ"
    try:
        ...
        if _is_letsencrypt_certificate(cert_path, key_path):
            auto_renew_msg = _ensure_certbot_auto_renew(domain)
        else:
            auto_renew_msg = "ใช้ Let’s Encrypt พร้อมต่ออายุอัตโนมัติ"
    ...
    return f"ตั้งค่า reverse proxy แล้ว ({domain} -> 127.0.0.1:{port}) | {auto_renew_msg}"
...
def remove_reverse_proxy(domain: str | None = None) -> str:
    ...
    try:
        if is_root_user():
            ...
            for hook in (CERTBOT_PRE_HOOK, CERTBOT_DEPLOY_HOOK, CERTBOT_POST_HOOK):
                try:
                    hook.unlink(missing_ok=True)
                except Exception:
                    pass
            try:
                CERTBOT_RENEW_CRON.unlink(missing_ok=True)
            except Exception:
                pass
            ...
            if sudo:
                ...
                for hook in (CERTBOT_PRE_HOOK, CERTBOT_DEPLOY_HOOK, CERTBOT_POST_HOOK, CERTBOT_RENEW_CRON):
                    subprocess.run([sudo, "-n", "rm", "-f", str(hook)], check=False, capture_output=True, text=True)
                subprocess.run([sudo, "-n", "systemctl", "reload", "nginx"], check=False, capture_output=True, text=True)
    except Exception as exc:
...
def web_status() -> dict[str, str]:
    ...
    domain_reachable = check_public_web_reachable(domain_url) if domain_url else False
    domain_tls_valid = check_public_web_ready(domain_url) if domain_url else False
    alive = pid_alive or endpoint_alive or domain_reachable
    return {
        "pid": str(pid or "-"),
        "pid_alive": "yes" if pid_alive else "no",
        "endpoint_alive": "yes" if endpoint_alive else "no",
        "domain_alive": "yes" if domain_reachable else "no",
        "domain_tls_valid": "yes" if domain_tls_valid else "no",
...
def print_web_status() -> None:
    ...
    print(f"TLS โดเมนเชื่อถือได้: {status['domain_tls_valid']}")
    ...
    if domain_value != "-" and status["domain_alive"] == "yes" and status["domain_tls_valid"] != "yes":
        print("หมายเหตุ: โดเมนตอบได้ แต่ใบรับรองยังไม่ผ่านการเชื่อถือของเบราว์เซอร์")
...
def bind_domain() -> None:
    ...
    if value:
        ...
        port = web_public_port()
        if not check_local_web_ready(port):
            print("เว็บต้นทางยังไม่พร้อม กำลังพยายามเริ่มเว็บอัตโนมัติก่อนตั้ง reverse proxy...")
            start_result = start_web_background()
            if start_result != 0:
                print("เริ่มเว็บอัตโนมัติไม่สำเร็จ ให้ตรวจ log ของเว็บอีกครั้ง")
        proxy_result = ensure_reverse_proxy(value)
        print(proxy_result)
        port_result = open_port_best_effort([80, 443, port])
        print(f"Firewall: {port_result}")
        print("หมายเหตุ: ถ้าใช้ Cloudflare แบบ Proxy หรือ DNS only ต้องมี origin ตอบได้จริงผ่าน nginx/reverse proxy บน 80/443")
    else:
