...
def app_version() -> str:
    configured = config_value("version", DEFAULT_CONFIG["version"])
    if configured and configured.lower() != "latest":
        return configured

    try:
        release = latest_release_info(github_repo())
...
def env_defaults(
    ...
    return {
        ...
        "BREVO_API_KEY": config_value("brevo_api_key", ""),
        "BREVO_SENDER_EMAIL": config_value("brevo_sender_email", ""),
        "BREVO_SENDER_NAME": config_value("brevo_sender_name", project or project_name()),
    }
...
def sync_env_metadata(
    ...
    *,
    ...
    current["BREVO_API_KEY"] = current.get("BREVO_API_KEY") or config_value("brevo_api_key", "")
    current["BREVO_SENDER_EMAIL"] = current.get("BREVO_SENDER_EMAIL") or config_value("brevo_sender_email", "")
    current["BREVO_SENDER_NAME"] = current.get("BREVO_SENDER_NAME") or config_value("brevo_sender_name", project or project_name())
